...
import de.gieskerb.classCraft.classes.ClassSpecificLUT.getMaxAge
...
class BlockBreakListener : Listener {
    ...
    fun onBlockBreak(event: BlockBreakEvent) {
        ...
        when (playerClass) {
            ...
            is FarmerClass -> {
                player.sendMessage( block.type.toString())
                if (farmerBlocks!!.contains(MaterialPair(block.type))) {
                    player.sendMessage("You have farmed the right Block!")
                    val maxAge: Int = getMaxAge(block.type)
                    if(block is Ageable && maxAge == block.age) {
                        player.sendMessage("When it was at max age")
                    }
                }
            }
