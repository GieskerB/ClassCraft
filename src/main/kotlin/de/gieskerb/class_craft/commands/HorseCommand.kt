...
class HorseCommand : CommandExecutor {
    private fun spawnHorse(player: Player): Horse {
        ...
        if (horseData != null) {
            horse.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)!!.baseValue = horseData.speed
            horse.getAttribute(Attribute.GENERIC_JUMP_STRENGTH)!!.baseValue = horseData.jump
            horse.setAdult()
            ...
            horse.getAttribute(Attribute.GENERIC_MAX_HEALTH)!!.baseValue = horseData.health
    ...
    override fun onCommand(sender: CommandSender, command: Command, string: String, args: Array<String>): Boolean {
        ...
        if (args.size == 1 && args[0] == "mount") {
            ...
            if (args.size == 2) {
                ...
                if (args[0] == "speed") {
                    horse!!.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED)!!.baseValue = args[1].toDouble()
                } else if (args[0] == "jump") {
                    horse!!.getAttribute(Attribute.GENERIC_JUMP_STRENGTH)!!.baseValue = args[1].toDouble()
                }
            }
        }
