...
public class MinerClass extends BaseClass {

    public final static String CLASS_NAME = "Miner";
    private static ItemStack displayItem = null;

    public MinerClass(Player player) {
        super(MinerClass.CLASS_NAME, player);
    }

    public MinerClass(JSONObject json, Player player) {
        super(json, player);
    }

    public static ItemStack getDisplayItem() {
        if (displayItem == null) {
            displayItem = new ItemStack(Material.NETHERITE_PICKAXE);
    ...
    public void reapplyRewardEffects() {
        if (super.getLevel() >= 17) {
            super.playerReference.addPotionEffect(new PotionEffect(PotionEffectType.HASTE, -1, 1, false, false));
        } else if (super.getLevel() >= 5) {
            super.playerReference.addPotionEffect(new PotionEffect(PotionEffectType.HASTE, -1, 0, false, false));
        }
        if (super.getLevel() >= 15) {
            Objects.requireNonNull(super.playerReference.getAttribute(Attribute.GENERIC_MAX_HEALTH)).setBaseValue(15);
        }
    }
    ...
    public void removePermanentEffects() {
        ...
        Objects.requireNonNull(super.playerReference.getAttribute(Attribute.GENERIC_MAX_HEALTH)).setBaseValue(20);
    }
