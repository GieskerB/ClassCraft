...
public class WarriorClass extends BaseClass {
    ...
    public final static String CLASS_NAME = "Warrior";
    private static ItemStack displayItem = null;

    public WarriorClass(Player player) {
        super(WarriorClass.CLASS_NAME, player);
    }

    public WarriorClass(JSONObject json, Player player) {
        super(json, player);
    }

    public static ItemStack getDisplayItem() {
        if (displayItem == null) {
            displayItem = new ItemStack(Material.NETHERITE_SWORD);
    ...
    public void reapplyRewardEffects() {
        if (super.getLevel() >= 17) {
            super.playerReference.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, -1, 1, false, false));
        } else if (super.getLevel() >= 5) {
            super.playerReference.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, -1, 0, false, false));
        }
